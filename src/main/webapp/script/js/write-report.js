...
$(function () {
	...
	app.controller('testCtrl', function ($scope, $http) {    
		...
		$http.post("/pcbmis/order/reportDetail?reportNum=" + loc).success(function (response) {
        	...
        	$scope.note=response.reportDetail.note.note.replace(new RegExp("<br>", "g"), "\n");  
        	$scope.supplierMistakeNote=response.reportDetail.note.supplierMistakeNote.replace(new RegExp("<br>", "g"), "\n");  
...
//             	}
	        	 ...
	        	 $("#microsectioning-judgeResult").val(response.reportDetail.report_content.microsectioning.judgeResult);
...
function writetijiao(){
	...
	if(flag){
		...
		console.log($("#acceptanceStandard").val());
		reportIntro.templateId=$("#acceptanceStandard").val().replace(/[^0-9]/ig,"");
	...
	//		reportApertureChecks[i].ID=$("#ID"+i+"").val();
		...
		var note={}
		note.note=$("#note_note").val().replace(new RegExp("\n", "g"), "<br>");
		note.supplierMistakeNote=$("#supplierMistakeNote").val().replace(new RegExp("\n", "g"), "<br>");
		...
		var templateId=$("#acceptanceStandard").val();
		$.ajax({  
	        type: "POST",  
	        url: "/pcbmis/report/postReport?reportNum=" + loc+"&templateId="+templateId.replace(/[^0-9]/ig,""),  
	        data:jsonstr,
