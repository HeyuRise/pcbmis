...
//		})
		...
		app.controller('testCtrl', function ($scope, $http) {   
	    	...
	    	$http.post("/pcbmis/order/detailNew?checkNum=" + loc).success(function (response) {
	        	...
	        	$scope.note = response.auditDetail.note.note.replace(new RegExp("<br>", "g"), "\n");  
	    		...
	    		if($("#submission_confirmation_form").form("validate")){
					...
					$.ajax({  
	    				 ...
	    				 success: function (data) {  
	    					 if(data.result==0){
	    						window.location = "num.html?bad=1&checkNum="+loc+"&edit="+loc;
	    					 }else{
...
//			    	burrs_require:$("#burrs_require").val(),
			 ...
			 if($("#save_judge").val()==""){
				 ...
				 $.ajax({  
				        type: "POST",  
				        url: "/pcbmis/check/checkDate?checkNum=" + loc+"&templateId="+templateId.replace(/[^0-9]/ig,""),  
				        data:jsonstr,
				        contentType: 'application/json', 
