...
import com.pcbwx.pcbmis.common.trust.MyX509TrustManager;
...
public class DingUtil {
	...
	// 获取企业钉钉的access token
	private final static String GET_ACCESS_TOKEN_URL = "https://oapi.dingtalk.com/gettoken?corpid=CORPID&corpsecret=SECRECT";
	//	通过code获取钉用户身份
	...
	// 获取jsapi_ticket
	private final static String GET_JSAPI_TICKET_URL = "https://oapi.dingtalk.com/get_jsapi_ticket?access_token=ACCESS_TOKEN";
...
//    	String url = ConfigProperties.getProperty(ConfigEnum.GET_ACCESS_TOKEN_URL);
    	String url = DingUtil.GET_ACCESS_TOKEN_URL;
    	url = url.replaceAll("CORPID", corpId);
    	url = url.replaceAll("SECRECT", secrect);
    	logger.info("获取钉钉acessToken：" + url);
    	return JSONObject.parseObject(httpRequest(url, "GET", null));
    }
    /**
     * 获取jsapi_ticket
     * @param account
...
//    	String url = ConfigProperties.getProperty(ConfigEnum.GET_JSAPI_TICKET_URL);
    	String url = DingUtil.GET_JSAPI_TICKET_URL;
    	url = url.replaceAll("ACCESS_TOKEN", accessToken);
    	logger.info("获取jsapi_ticket：" + url);
    	return JSONObject.parseObject(httpRequest(url, "GET", null));
    }
    /**
     * 获取钉用户的身份
